...
if [ "$status" -eq 0 ]; then
  power_msg="Digest completed successfully, powering off."
else
  power_msg="Digest failed (exit $status), powering off anyway."
fi

/usr/bin/logger -t day-news "$power_msg"
printf '%s %s\n' "$(date -Is)" "$power_msg" >> "$LOG_FILE"
